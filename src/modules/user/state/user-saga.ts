...
export function* authSaga() {
  ...
  yield takeLatest(
    USER_ACTION.LOGIN_GOOGLE,
    function* callBack(action: IAction) {
      ...
      if (accessToken) {
        ...
        yield put({
          type: USER_ACTION.LOGIN_GOOGLE_SUCCESS,
          payload: userInfo,
        });
        navigate('/matches');
      }
  ...
  yield takeLatest(
    USER_ACTION.LOGIN_FACEBOOK,
    function* callBack(action: IAction) {
      yield console.log(action.payload.response);
      const { accessToken, email, name, userID, picture } =
      ...
      if (accessToken) {
        ...
        console.log(userInfo);
        yield put({
          type: USER_ACTION.LOGIN_FACEBOOK_SUCCESS,
          payload: userInfo,
        });
        navigate('/matches');
      }
